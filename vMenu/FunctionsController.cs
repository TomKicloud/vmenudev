...
namespace vMenuClient
{
	...
	class FunctionsController : BaseScript
	{
	    ...
	    private readonly Dictionary<string, string> _streetNameReplacements = new Dictionary<string, string>()
	    {
	        {"Great Ocean Hwy", "Great Ocean Hwy [US Route 1]"},
	        {"Alta St", "Alta St [SR-19]"},
	        {"West Eclipse Blvd", "Eclipse Blvd [SR-18]"},
	        {"Eclipse Blvd", "Eclipse Blvd [SR-18]"},
	        {"Vinewood Blvd", "Vinewood Blvd [SR-18]"},
	        {"Mirror Park Blvd", "Mirror Park Blvd [SR-18]"},
	        {"El Rancho Blvd", "El Rancho Blvd [SR-18]"},
	        {"Boulevard Del Perro", "Del Perro Blvd [SR-17]"},
	        {"Hawick Ave", "Hawick Ave [SR-17]"},
	        {"Del Perro Fwy", "Del Perro Fwy [I-2]"},
            {"North Rockford Dr", "Rockford Dr [SR-14]"},
	        {"South Rockford Dr", "Rockford Dr [SR-14]"},
	        {"Dutch London St", "Dutch London St [SR-22]"},
	        {"Route 68", "US Route 68"},
	        {"Tongva Dr", "Tongva Dr [US Route 11]"},
	        {"Senora Fwy", "Senora Fwy [US Route 13]"},
	        {"Los Santos Freeway", "Los Santos Freeway [I-1]"},
	        {"Palomino Fwy", "Palomino Fwy [US Route 15]"},
            {"Olympic Fwy", "Olympic Fwy [I-4]"},
	        {"Elysian Fields Fwy", "Elysian Fields Fwy [US Route 20]"},
	        {"La Puerta Fwy", "La Puerta Fwy [I-5]"},
	        {"Miriam Turner Overpass", "La Puerta Fwy [I-5]"}
        };
        ...
        private async Task UpdateLocation()
		{
			if (MainMenu.MiscSettingsMenu != null)
			{
				if (MainMenu.MiscSettingsMenu.ShowLocation && cf.IsAllowed(Permission.MSShowLocation))
				{
				    ...
				    if (_streetNameReplacements.ContainsKey(crossingName))
				    {
				        if (_streetNameReplacements.TryGetValue(crossingName, out string newCrossingName))
				        {
		...
		private void ShowLocation()
		{
			...
			string currentStreet = World.GetStreetName(currentPos);

		    if (_streetNameReplacements.ContainsKey(currentStreet))
		    {
		        if (_streetNameReplacements.TryGetValue(currentStreet, out string newCurrentStreet))
		        {
		            currentStreet = newCurrentStreet;
		        }
		    }
